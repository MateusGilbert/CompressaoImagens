...
//talvez as duas funcoes sejam desnecessarias
//conferir!!!
...
int avg_rem(int **img, int x, int y){
	double avg = 0;
	for (int i=0; i<x; i++)
		for (int j=0; j<y; j++)
			avg += (double) img[j][i];
	avg = (int) subd_round( ((double) avg) / (x*y) );

	for (int i=0; i<x; i++)
		for (int j=0; j<y; j++)
			img[j][i] = (int) subd_round((double) img[i][j] - avg);

	return avg;
}

void avg_add(int **img, int x, int y, int avg){
	for (int i=0; i<x; i++)
		for (int j=0; j<y; j++)
			img[j][i] = (int) mpel(subd_round((double) img[i][j] + avg));
