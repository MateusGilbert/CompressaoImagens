...
void
sub(int **Image_orig, int **Image_out, int **Image, int xsize, int ysize)
{
...
//printf("\nying= %d", yimg);
 ...
 for(y=0;y<yimg;y++)//troquei ylum por yimg
	{
	 if ((pSY[y]=(unsigned char *)malloc(ximg))==NULL)
		{
         printf("Memory allocation for luminance transform failed at line %d"
																		   ,y);
		 exit(1);
        }
    }
...
//			cerr<<pSIMG[y][x]<<endl;
		 ...
		 for (x=bband[s][0].xmin;x<bband[s][0].xmax;x++)
			{
			 if (s==0)
				{
			     *(pSY[y]+x) = mpel(subd_round(*(pSIMG[y]+x)/
												  pow(2.0,(double)NSTAGES)));
					/*cerr<<pSY[y][x]<<" - "<<pSIMG[y][x]<<endl;*/
				}
			 else
				{
			     *(pSY[y]+x) = mpel(subd_round(*(pSIMG[y]+x)/
 ...
 printf("\nStoring coded subband decomposition...");
	...
	/*std::cout<<"xlum = "<<ylum<<" yimg = "<<yimg<<std::endl;*/
 for (y=0; y<ylum;y++){
	for(x=0; x<ximg; x++){
		Image[y][x]=pSY[y][x];
		/*cerr<<Image[y][x]<<endl;*/
		/*Image[y][x] = pISIMG[y][x];*/
	}
 }
...
//void
//sub_anal(int **Image_orig, int **Image_out, int **Image, double **Wav_Img, int xsize, int ysize)
//{
//
//// int sub4anal(); /* function that performs two-band subband analysis */
//// int sub4synt(); /* function that performs two-band subband analysis */
...
void
analysis(int **Image_orig, int **Image, double *pSIMG[YIMG], int xsize, int ysize)
{
...
void
synthesis(double *pSIMG[], int **Image_out, int xsize, int ysize){
 int x,y; /* hold position into images */
