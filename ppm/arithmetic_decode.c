...
void
start_decoding(FILE *source)
{ 
 int i;
 value = 0;				               
 for (i=1; i<=Code_value_bits; i++) 	
	{	   								/* input bits to fill the 		*/
     value = 2*value+input_bit(source);		/* code value    				*/
    }
...
int decode_symbol(int cum_freq[], FILE *source)
 ...
 for (;;) 						            	/* Loop to get rid of bits	*/
    {
     ...
     value = 2*value+input_bit(source);				/* Move in next input bit	 */
    }
 return(symbol);
}
